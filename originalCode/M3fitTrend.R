# Author: Slawek Smyl 
# Oct 2016
# An example fitting trend-only, Gaussian error, homoscedastic version of LGT to one of the M3 yearly curves
###############################################################################
...
#non-seasonal model
parameters = c("l", "b", "sigma", "levSm",  "bSm", "coefTrend", "powTrend", "locTrendFract") 
modelX = '
	data {  
...
fitTrend <- function(y,maxPredictionHorizon=6) {
  n=length(y)
...
lines((n+1):(n+maxPredictionHorizon),avgYfs[2,], col='blue',lwd=2)# median
