# Non-Seasonal Local Global Trend homoscedastic (LGTh) algorithm
# Dec 2016
# Slawek
#############################################
...
predictionAlgorithm=paste('LGTh');
...
modelX = '
	...
	model {
		...
		for (t in 2:N) {
			y[t] ~ student_t(nu, l[t-1]+coefTrend*fabs(l[t-1])^powTrend+locTrendFract*b[t-1], sigma);
		}
	}
'    
