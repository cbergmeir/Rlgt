# Non-Seasonal Local Global Trend (LGTSkew) algorithm, skew
# Dec 2016
# Slawek
#############################################
...
predictionAlgorithm=paste('LGTSkew');
...
modelX = '
  functions { 
    ...
    bInit ~ normal(0,CAUCHY_SD);
		...
		for (t in 2:N) {
			y[t] ~ skew_student_t(nu, l[t-1]+coefTrend*fabs(l[t-1])^powTrend+locTrendFract*b[t-1], 
				sigma*fabs(l[t-1])^powx+ offsetSigma, SKEW);
		}
	}
'    
