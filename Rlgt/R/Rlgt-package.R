...
#' \deqn{  y_{t+1}=l_{t}+ \gamma l_{t}^{ \rho }+ \lambda b_{t}   }  (eq. 1.2)
#' \deqn{  l_{t}= \alpha y_{t}+ \left( 1- \alpha  \right)  \left(  l_{t-1} \right)    }  (eq. 1.3)
#' \deqn{  b_{t+1}= \beta  \left( l_{t+1}-l_{t} \right) + \left( 1- \beta  \right) b_{t}  }  (eq. 1.4)
#' \deqn{   \sigma _{t+1}= \sigma l_{t}^{ \tau}+ \varsigma    } (eq. 1.5)
#' and the notations are defined as follow:
#' 
#' \deqn{  y_{t}   } : value of the dependent variable of interest at time t
#' \deqn{  y_{t+1}   } : predicted value of y at time t+1 given information up to time t
#' \deqn{   \sigma _{t}   } : variance of the distribution at time t
#' \deqn{  l_{t}   } : level at time t
#' \deqn{  b_{t}   } : local trend at time t
#' 
#' Parameters of the model which need to be estimated
#' \deqn{  v   } : degrees of freedom of the t-distribution
#' \deqn{   \gamma    } : coefficient of the global trend
#' \deqn{   \rho    } : power coefficient of the global trend
#' \deqn{   \lambda    }: damping coefficient of the local trend
#' \deqn{   \alpha    } : smoothing parameter for the level
#' \deqn{   \beta    } : smoothing parameter for the local trend
#' \deqn{   \sigma    } : coefficient of the heteroscedastic standard deviation
#' \deqn{   \tau} : power coefficient of the heteroscedastic standard deviation
#' \deqn{   \varsigma} : base/ minimum value of the standard deviation

#' The rationale of each individual equation of the model is discussed below.

#' \strong {eq. 1.1. Student’s t error distribution}
...
#' \strong {eq. 1.5. Heteroscedastic Error }
...
#' \strong {eq. 1.2: One-step prediction forecast}
...
#' The last term of the right-hand side of eq.1.2. refers to the usual local dampen trend in ETS model. However, there is an addition of dampen parameter  \eqn{  \lambda  } , constrained such that  \eqn{ -1<  \lambda  <1 } , to reduce the strength of the local trend model. \par

#' \strong {eq. 1.3: Level adjustment equation}
#' This equation is defined according to the classical linear trend ETS model. The level at time t ( \eqn{ l_{t} } ) is calculated as a weighted average of the current observation  \eqn{ y_{t} }  and the previous level at lag 1 ( \eqn{ l_{t-1} } ) with smoothing parameter  \eqn{  \alpha  } .\par


#' \strong {eq. 1.4: Local trend adjustment equation}


#' Similarly, the evolution of the local trend  \eqn{ b_{t}  } is identically defined to the linear trend method.\  The local trend at time t  \eqn{ b_{t} }  is obtained as a weighted average of the difference in level terms  \eqn{  \left( l_{t}-l_{t-1} \right)  }  and the trend at time t-1 ( \eqn{ b \_  \left( t-1 \right)  \right)  }  with the smoothing parameter  \eqn{  \beta  } .\par
...
#' Note that a more elaborate explanation of these prior choices will also be given in Chapter 5.
#' 
#' 
#' 
NULL
