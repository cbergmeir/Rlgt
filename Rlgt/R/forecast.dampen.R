...
# library(sn)
forecast.dampen <- function(object, h=ifelse(frequency(object$x)>1, 2*frequency(object$x), 10),
  level=c(80,95),
  NUM_OF_TRIALS=2000, 
  ...
  for (irun in 1:NUM_OF_TRIALS) {
    ...
    #t=1
    if (SEASONALITY>1) { # For seasonal
      sS[1:SEASONALITY]=s[indx,(ncol(s)-SEASONALITY+1):ncol(s)]
      for (t in 1:h) {
        seasonA=sS[t]
        ## From eq.6.a
        expVal=(prevLevel + psiS* bS)*seasonA;
