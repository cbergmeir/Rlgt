...
transformed parameters {
	...
	for (t in 2:N) {	
		...
		if (SEASONALITY_TYPE==0) {//HW
			expVal[t]=l[t-1]*s[t] + r[t];   //expVal[t] can't use y[t] or  anything derived from it
			newLevelP=(y[t]-r[t])/s[t];
		} else { //if (SEASONALITY_TYPE==1) 
			expVal[t]=l[t-1] + s[t]*l[t-1]^powSeason + r[t];  //expVal[t] can't use y[t] or  anything derived from it 
...
model {
	...
	for (t in 2:N) {
	  if (USE_SMOOTHED_ERROR==0)
	  	y[t] ~ normal(expVal[t], offsetSigma);  // expVal[t]^powx , not l[t-1]^powx
	  else
