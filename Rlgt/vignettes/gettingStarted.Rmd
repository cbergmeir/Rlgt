...
The nonlinear global trend allows to model well a frequent situation in business where series grow faster than lineraly, but slower than exponentially. The global trend can also fit well a damped trend.
...
The models are fitted using Markov Chain Monte Carlo with RStan package. One of the consequences is that their computational requitements are 2-3 orders of magnitude higher in comparison to e.g. ETS from the  Forecast package. So, while these models are often more precise than the classical time series algorithms, they are not suitable for a quick forecasting on a large number of time series.

The current version of the models supports time series data with positive data values. Many time series data fall into this category, while for others, a simple transformation can be performed to obtain the required positive time-series data. Therefore, the constraint should not be too restrictive for most general applications.

 
 
## LGT (Local and Global Trend)

It generalizes Holt Linear method, so it is meant to be used on non-seasonal series, like yearly. It has two trends: local linear and global nonlinear. Together these trends can model well situations of short local trends that may go against the long trend, but eventually will be overriden by the latter.

###Fitting
For example, let's forecast last 10 points from BJsales dataset:
...
model <- rlgt(train) 
```
All models of the package are fitted using rlgt() function. As a minimum, like in this example, we need to pass the time series. 
...
After several minutes, somethimes more :-), we can proceed with with 

###Forecasting
```
forec= forecast(model, h = horizon)
```
The object returned by forecast is compatible with the Forecast package, and when printed looks like this:
```
...
```
So, it has point forecast and a number of prediction intervals. Those you see, were produced with default values of *level* paramter of forecast function: c(80,95). To see all the parameters, execute 
```
?forecast.rlgtfit
```
Forecasting is much quicker than the model fitting, so you could change e.g. the *level* values and rerun the forecast:
```
forec= forecast(model, h = horizon, level=c(90,98))
```
However, when requesting high coverage values, it may be prudent to increase the default number of simulation performed (default is 2000);
```
...
Numerical results will vary a bit, it is Monte Carlo after all:
```
...
## LGT with regression
In this release we implemented an additive regresion, but it is a new, experimental feature. The regression works for all models, not just LGT. You need to provide a matrix [N,d] of regressors for fitting, where N is length of the series, and d dimensionality of the regressors. And you need to provide matrix [h,d] of regressors for forecasting, where h is the forecasting horizon. 
In example below we create the regression matrix with two columns that are lagged (by 3 and 4) values of BJsales.lead series. 
```
theDataSet=BJsales 
...
We are running on `r Rlgt::rlgt.control()$NUM_OF_CHAINS` concurrent chains, here each executing 5000 warmup and 5000 sampling iterations, so together 20K od sampling iterations, therefore 18K of divergent transitions are certainly too many. But checking the main paramters   
```
...
It generalizes Holt-Winters method, so it is meant to be used on seasonal series, like monthly. The local linear trend of LGT is removed and replaced with multiplicative (by default) or generalized seaonality. The generalized seasonality has an adaptive behavior, spanning additive and multiplicative seasonalities, but it takes longer to fit.   
Let's forecast Monthly Airline Passenger Numbers, the AirPassengers time series, last 24 of its monthly values:
```
...
```
AirPassengers is a time series of frequency 12, an object of class ts(). However, any subsetting, as done here to get train and actuals, converts result to a pure numeric vector. Therefore, mostly to get nice time series-style plots, we convert them back to time series:
```
train=ts(train, start=tspOrg[1], frequency=tspOrg[3])
tspt=tsp(train)
actuals=ts(actuals, start=tspt[2]+1/tspt[3], frequency=tspt[3])
```
We fit the model using "seasAvg" method for calculating level. It replaces the default Holt-Winters-style formula for the level update with a moving average of the last *seasonality* points. It can speed up calculations without reducing accuracy in sitations where the trend is stable or changing in a stable manner.
```
rstanmodel <- rlgt(train,  
		level.method="seasAvg",
		control=rlgt.control(NUM_OF_ITER=10000, MAX_NUM_OF_REPEATS=1))  
```
Please note that *seasonality* is not directly specified - it can be extracted from the time series input.   
Then we forecast and plot:
```
...
Multi-seasonal time series, for example half-hourly data of electricity usage that has both daily (48) and weekly seasonalities (48\*7), can be modelled with SGT using *seasonality* equal to largest frequency of the series, here 336. We will demonstrate it by using Taylor data set and forecasting one week ahead:
```
theDataSet=taylor
...
```
And then as usual, forecasting, plotting, and error calculations:
```
forec= forecast(rstanmodel, h = length(actuals))
...
S2GT an experimental model that is an extension of SGT following Taylor 2003. 
