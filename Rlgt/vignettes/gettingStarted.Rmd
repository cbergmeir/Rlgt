...
After several minutes, sometimes more :-), we can proceed with
...
```
So, it has point forecast and a number of prediction intervals. Those you see, were produced with default values of *level* parameter of forecast function: c(80,95). To see all the parameters, execute 
```
?forecast.rlgtfit
```
...
## LGT with regression
In this release we implemented an additive regression, but it is a new, experimental feature. The regression works for all models, not just LGT. You need to provide a matrix [N,d] of regressors for fitting, where N is length of the series, and d dimensionality of the regressors. And you need to provide matrix [h,d] of regressors for forecasting, where h is the forecasting horizon. 
...
We are running on `r Rlgt::rlgt.control()$NUM_OF_CHAINS` concurrent chains, here each executing 5000 warmup and 5000 sampling iterations, so together 20K od sampling iterations, therefore 18K of divergent transitions are certainly too many. But checking the main parameters   
```
...
It generalizes Holt-Winters method, so it is meant to be used on seasonal series, like monthly. The local linear trend of LGT is removed and replaced with multiplicative (by default) or generalized seasonality. The generalized seasonality has an adaptive behavior, spanning additive and multiplicative seasonalities, but it takes longer to fit.   
