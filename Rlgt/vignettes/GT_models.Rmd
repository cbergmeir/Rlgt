...
* $\gamma$  :coefficient of the global trend, between -1 and 1, usually between 0 and 1
* $\rho$  :power coefficient of the global trend, between 0 and 1
...
* **eq. 1.2:  Expected value for next step**
    ...
    The last term refers to the usual local trend in ETS model. However, there is an addition of dampening parameter $\lambda$, constrained such that  $-1<\lambda<1$, to reduce the strength of the local trend model. Although we allow $\lambda$ to be negative, and potentially have some counter-trending impact, in practice it is almost always >0.
...
### "Smoothed innovation" error size function
...
$$y_{t+1} \sim Student \left( \nu,\widehat{y}_{t+1}, \sigma _{t+1} \right)  \quad (eq. 2.1) $$
$$\widehat{y}_{t+1}= l_{t}+ \gamma l_{t}^{\rho} + s_{t+1}l_{t}^{\tau} \quad (eq. 5.2)$$
$$l_{t+1}= \alpha \left( y_{t+1}-s_{t+1}l_{t}^{\tau} \right) + \left( 1- \alpha  \right) l_{t} \quad (eq. 5.3)$$ 
$$s_{t+m+1}= \zeta  \frac{y_{t+1}-l_{t}-\gamma l_{t}^{\rho}}{l_{t}^{\tau}}+ \left( 1- \zeta  \right) s_{t+1}  \quad (eq. 5.4)$$

$$\widehat{\sigma}_{t+1}= \sigma \widehat{y}_{t+1}^{ \tau}+ \xi \quad (eq. 2.5)$$

#### Additional parameters
 
* $\tau$  : power coefficient of the generalized seasonality, between 0 and 1
...
* **eq. 5.4. Seasonality update formula**
