...
## Introduction  
This vignette provides an overview of the global trend time series forecasting models. The "LGT" model stands for Local and Global Trend and is devised to forecast non-seasonal time series. Meanwhile, the "SGT" model stands for Seasonal Global Trend model and is devised to complement the former on seasonal time series. The combination of these models were tested on the 3003 time-series M3-competition dataset, and they have been shown to outperform all of the models originally participating in the competition by a significant margin. It is quite likely that L/SGT is still the most accuarate algorithm on the M3 data sets. 
...
Additionally, the package provides S2GT model that extends SGT to deal with two seasonalities, following Taylor (2003).

The models can be seen as extensions of following Exponential Smoothing models: Holt Linear Method (LGT), Holt-Winters model (SGT), and Taylor dual seasonalit model (S2GT). The main differences are as follows:
...
The package also provies some variants of the base algorithms, they are described in later sections.

The models are Bayesian and are fitted using Markov Chain Monte Carlo with RStan package. The user can "nudge" the models using many hyperparamters of the parameters' prior distributions, altough is is usually not necessary. While Hamiltonian Monte Carlo engine of Stan system is relatively fast, computation time required for a single time series is typically between a few to 20 minutes. A substantially longer computation time is likely a sign of not a good match of the chosen model to the time series.
The current version of the models only supports time series data with entirely positive data values. Many time series data fall into this category, while for others, a simple transformation can be performed to obtain the required positive time-series data. Therefore, the constraint should not be too restrictive for most general applications.
...
* $\widehat{\sigma}_{t+1}$  :size of error at time t+1
...
* **eq. 1.2:  Expected value for next step**
    ...
    The expected value for the next step is modeled as a sum of level $l_{t}$, global nonlinear trend $\gamma l_{t}^{ \rho }$, and damped local linear trend $\lambda b_{t}$. We use term "global" in the nonlinear trend description to stress the fact that both $\gamma$ and $\rho$ are fitted on whole of the time series and do not change in time. Typically the global trend will dominate over the short-term behavior caused by the local linear trend. 
    ...
    The last term refers to the usual local trend in ETS model. However, there is an addition of dampening parameter $\lambda$, constrained such that  $0<\lambda<1$, to reduce the strength of the local trend model. 
 
* **eq. 1.5. Heteroscedastic error**

    Size of the error is modelled with a monotonically growing function of the expected value. Typically, the size of the error grows in absolute terms as the level and expected value grow, but due to averaging effects, it gets proportionally smaller - this formula models well such behavior. Please note that in extreme cases: when $\tau$ is close to 0, the error size is constant, and when $\tau$ gets cose to 1, the error size is proportional to the expected value, and thus approaches behavior of the multiplicative error.

* **eq. 1.3: Level adjustment formula**

    This formular is very similar to that of Halt Liner Method, except that it does not include local nor global trends.

* **eq. 1.4: Local trend adjustment formula**

    The same as in Halt Linear Method.
...
The model is similar to the non-seasonal LGT model described above. There are couple of modifications:
...
When the error size appears to have its own dynamics that isn't just reflection of the current expected value of the series, e.g. during periods of high-volatility on stock markets, a better function for error size may be one that is proportional to an average of absolute values of recent "innovations" or errors. The formula is:
...
* **eq. 5.2. Expected value formula**
    ...
    Instead of multiplying by $s_{t+1}$ we are adding $s_{t+1}l_{t}^{\tau}$. It has interesting consequences: if ${\tau}$ is getting close to 0, $l_{t}^{\tau}$ gets close to 1 and then the whole term gets close to $s_{t+1}$ and we get the additive seasonality. However on the opposite end, when ${\tau}$ is getting close to 1, as in case of the nonlinear trend, we are getting the compound formula and the seasonality similar to the multiplicative one. Therefore the formula provides for the behavior that falls between additive and multiplicative seasonality.
    
Formulas for S2GT are modified along similar lines.


### 3. "Seasonal average" method for calculating level
This is an alternative method for calculating level in the seasonal models (SGT and S2GT). Instead of using Holt-Winters' style way, as in eq.2.3 and 3.3, the level is calculated as a moving average of last SEASONALITY points in case of SGT, and SEASONALITY2 points in case of S2GT. This apprach makes sense for series with stable or steadily changing level, and possible unimportant occasional flare-ups. The benefits ◄may include faster calculation and better accuracy.


## Regression
All model support additional regressors. The data needs to be passed as matrix (with number of columns equal to the number of regressors) to both rlgt (for training) and forecast functions. 


## Prior Distributions 
 
The prior distributions of parameters have their hyper parameters documented and available for tweaking, although most of the time it is not required. Perhaps the most likely hyperparameter to change would be POW_TREND_ALPHA which is the alpha parameter of Beta distribution of the global trend power coefficient. Occasionally it may be useful to increase it above 1, to say 3-6, to encourage higher curvature of the global trend. 

The prior distributions of the parameters and their default limits (which can also be changed) are listed below:

* $\gamma$ : Cauchy distribution with minimum value of 0 and the scale parameter equals to 1/200 of the maximum value of y
* $\sigma$ :  Cauchy distribution with location value of 0 and the scale parameter equals to 1/200 of the maximum value of y
* $\xi$ : Cauchy distribution with minimum value of MIN_SIGMA and the scale parameter equals to 1/200 of the maximum value of y
* $\rho$  :  Scaled (to the default range of -0.5 and 1.0) Beta distribution
* $\alpha, \beta, \eta, \lambda, \zeta, \delta, \tau, \theta$:  Uniform distribution between 0 and 1
* $v$    : Uniform distribution between 2 and 20
Regression coefficients are drawn from (appropriately scaled) Cauchy distribution centered on 0. 

## Additional notes

### Non-integer seasonalities
  The seasonal models support specifying non-integer seasonalities. Internally it is done through a partial allocation of newly calculated seasonality coefficients.
